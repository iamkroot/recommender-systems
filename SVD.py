...
class SVD:
    ...
    Sigma is an m X n rectangular diagonal matrix, with each diagonal element as the singular values of the utility matrix.
    V is an n X n unitary matrix.
    """
    def __init__(self):
        self.umDf = pd.read_csv("./data/utility_matrix.csv",low_memory=False)
        self.um = self.umDf.iloc[1:, 1:].values
        self.movies = list(self.umDf.columns)

    def generate_svd_matrices(self):
        """
        Normalizes the Utility matrix consisting of users, movies and their ratings by replacing 0s in a row
        by their row mean.
    ...
    def get_dimensions_for_x_energy(self, S, fraction, initial):
        """
        Finds the number of the dimensions to which Sigma matrix can be reduced to, so as to preserve (fraction * 100)% of the energy.
        ...
        Returns:
            dim (int) : The number of dimension that will preserve atleast (fraction * 100)% of the energy.
        """
        S = S[:initial]
    ...
    def predict_and_find_error(self, test_ratings):
        ...
        Also displays the Root Mean Square Error, Mean Absolute Error values and the prediction time for each component.
        
        Args:
            test_ratings (np.ndarray): An array of <user_id, item_id, true_rating> tuples
        """
        start = time.time()
        ...
        print("RMSE: ", rmse)
        print("MAE: ", mae)
        print("Prediction and error calculation of complete SVD took " + str(time.time()-next_part) + " secs")
        ...
        print("RMSE: ", rmse)
        print("MAE: ", mae)
        print("Prediction and error calculation of SVD with 90% energy took " + str(time.time()-next_part) + " secs")
